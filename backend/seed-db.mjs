...
try {
  ...
  for (const concern of concerns) {
    const stmt = db.prepare(
      'INSERT OR IGNORE INTO Concern (name, description, keywords, category, severity) VALUES (?, ?, ?, ?, ?)'
    );
    stmt.run(concern.name, concern.description, concern.keywords, concern.category, concern.severity);
  }
